...
local function get_primary_key(tbname)
	local sql = "select k.column_name column_name " ..
...
local function load_data_impl(config, uid)
	...
	while true do
		if not uid then
			...
			if not config.columns then
				sql = string.format("select * from %s where %s = %d order by %s asc limit %d, 1000", tbname, config.key, uid, pk, offset)
			else
				sql = string.format("select %s from %s where %s = %d order by %s asc limit %d, 1000", config.columns, tbname, config.key, uid, pk, offset)
			end
		end
		...
		if tbname == "d_score" then
			for i = 1, #rs do
				do_redis({ "zadd", "ladder", tonumber(rs[i].score), rs[i].id}, uid)
			end
		end
...
local function load_update_data()
...
local function isnumber(str)
	if str == nil or str == "" or #str > 12 then
		return str
	end
	for i = 1, #str do
...
local function make_pairs_table(t, fields)
	...
	if not fields then
		for i=1, #t, 2 do
			data[t[i]] = isnumber(t[i+1])
		end
	else
		for i=1, #t do
			data[fields[i]] = isnumber(t[i])
		end
	end
...
function CMD.start(config, user, common, update)
...
-- 从redis获取update类型表数据
function CMD.get_update(tbname, cfgid)
	local data = {}
	local config = update_table[tbname]
	local indexkeys
	if cfgid then
		indexkeys = do_redis({ "keys", tbname .. ":index:" .. cfgid })
	else
		indexkeys = do_redis({ "keys", tbname .. ":index:*" })
	end
	local keys = {}
	for _, indexkey in pairs(indexkeys) do
		local ids = do_redis({ "zrange", indexkey, 0, -1 })
	...
	if table.empty(keys) then
		if cfgid then
			keys = do_redis({ "keys", tbname .. ":" .. cfgid})
		else
			keys = do_redis({ "keys", tbname .. ":*" })
		end
	end
...
function CMD.add(tbname, row, typ, nosync)
	...
	elseif typ == 4 then
		config = update_table[tbname]
	end

	local uid
	if row.uid and typ == 2 then
		uid = row.uid
	end
	...
	if not nosync then
		...
		skynet.call("dbsync", "lua", "sync", sql, true)
	end
...
function CMD.update(tbname, row, typ, nosync)
	local config 
	if typ == 1 then
		config = config_table[tbname]
	elseif typ == 2 then
	...
	if not nosync then
		...
		for k, v in pairs(row) do
			if type(v) == "boolean" then
				setvalues = setvalues .. k .. "=" .. (v and 1 or 0 ) .. ","
			else
