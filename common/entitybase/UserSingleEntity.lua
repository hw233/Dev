...
function UserSingleEntity:add(record, nosync)
	if record.id and self.recordset[record.id] then return end		-- 记录已经存在，返回
	...
	if ret then
		self.recordset[record.id] = record
	end
	return ret, record
end
...
function UserSingleEntity:remove(record)
	if not record.id or not self.recordset[record.id] then return end		-- 记录不存在，返回
	self.recordset[record.id] = nil
