...
local function launch_slave(auth_handler)
	local function auth(fd, addr)
		...
		if hmac ~= crypt.base64decode(response) then
			skynet.error("ooooooo, hmac:", hmac, "response:", crypt.base64decode(response))
		...
		local ok, result, server, uid = pcall(auth_handler, token)

		return ok, result, server, uid, secret
	end
...
local function accept(conf, s, fd, addr)
	...
	local ok, result, server, uid, secret = skynet.call(s, "lua",  fd, addr)
	...
	elseif result == 0 then
		LOG_DEBUG("402 Password")
		write("response 402", fd, "402 Password")
		error("402 Password")
	end
