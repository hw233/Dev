...
local user_dc
local info_dc
local current_room_player = 0			--当前房间人数
...
---根据uid获取房间信息、房间主键
local function getroom_roomkey(uid)
...
local function exitroom(uid)
	...
	current_room_player = math.max(0, current_room_player - 1)

	return true
end

---销毁房间
---@param param boolean true表示给房主也发送
local function destroyroom(uid, param)
	local roomtype = userroomtype[uid]
	local rooms = getroom(roomtype.roomnum, roomtype.roomtype)
	...
	for i = 1, #rooms[roomid] do
		...
		if not param and uid ~= roommateid then
	...
	current_room_player = math.max(0, current_room_player - #rooms[roomid])
	rooms[roomid] = nil
...
local function createorjoinroom(stageid, uid, roomnum, roomtype)
	...
	--战斗人数验证
	local code
	if info_dc.req.getvalue(zerotime(), "battlenum") + current_room_player >= tonumber(skynet.getenv("max_battle_number")) then
		return errCodeDef.eEC_roomplayer_full
	else
		code = errCodeDef.eEC_success
	end	

	if roomnum == teammatchdefine.eroomtype.one then
		if roomtype ~= teammatchdefine.missiontype.pvp then
	...
	elseif roomnum == teammatchdefine.eroomtype.hundred then
		findroomret = findfitroom(hundredrooms, roomnum, stageid, uid)
		rooms = hundredrooms
	end
	...
	if findroomret == -1 then
		...
		if roomnum == teammatchdefine.eroomtype.hundred then
			...
			skynet.timeout(3000, function ()
				...
				local roomtype = userroomtype[uid]
				if roomtype then
					local rooms, roomkey = getroom_roomkey(uid)
					--用户退出，由于定时器无法取消，not beginbattle_time[roomid] or beginbattle_time[roomid] < os.time()判断是否为无效定时器
					--有可能用户换了房间，所以根据房间判断不全面
					if not roomkey or not next(rooms[roomkey]) or not beginbattle_time[roomid] or beginbattle_time[roomid] > tonumber(os.time()) then
						return
					end
					...
					--如果战斗服务器关闭，则踢此房间全部玩家
					if data == errCodeDef.eEC_battleser_isoff then
						destroyroom(uid, true)
					end
	...
	if roomid ~= -1 then
		userroomtype[uid] = {roomnum = roomnum, roomtype = roomtype}
	end
	current_room_player = current_room_player + 1

	return code, roomid, room
end
...
function response.getroom_and_uids(uid)
	local uids = {}
	local rooms, roomkey = getroom_roomkey(uid)
	if not next(rooms) or not roomkey then return uids end
	local room = rooms[roomkey]
	for i = 1, #room do
		table.insert(uids, room[i].playerid)
	end
	return uids
end
...
function response.maketeam(data)
	...
	--看似多余，实则对客户端参数进行校验
	if teammatesnum == teammatchdefine.eroomtype.one then
		roomnum = teammatchdefine.eroomtype.one
	elseif teammatesnum == teammatchdefine.eroomtype.two then
		roomnum = teammatchdefine.eroomtype.two
	elseif teammatesnum == teammatchdefine.eroomtype.three then
		roomnum = teammatchdefine.eroomtype.three
	elseif teammatesnum == teammatchdefine.eroomtype.ten then
		roomnum = teammatchdefine.eroomtype.ten
	elseif teammatesnum == teammatchdefine.eroomtype.hundred then
		roomnum = teammatchdefine.eroomtype.hundred
	else
		msg.code = errCodeDef.eEC_err_param
		return msg
	end
	
	msg.code, roomid, room = cs(createorjoinroom, stageid, uid, roomnum, stage.Task_Type)
