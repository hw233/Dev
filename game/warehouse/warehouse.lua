...
--Trie
--铭感词用args替换
function response.match_trie(str, args)
    if not str or not args then
        return
    end
	return trienode:match(str, args)
end

--参数中是否包含敏感词
function response.matchbool_trie(args)
    if not args then
        return false
    end
	return trienode:matchbool(args)
end

---初始化前缀树
---将所有敏感词加入树中
local function init_trie()
    trienode = trie.create()
    local sen = sharedata.deepcopy("sensitive_word")
    for k, v in ipairs(sen) do
        trienode:insert(v.word)
    end
end
--
